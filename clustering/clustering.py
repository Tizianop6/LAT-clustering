...
def main():
    #vvvvvvvvvvvvvvvvvvvvvvvvvvvvv INSERIRE QUI SOTTO NOME DEL FRB SU CUI FARE IL CLUSTERING
    NOMEFRB='FRB121102'
    #^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    global psf_camp
    global EN_camp
    ...
    #cerco l'indice della lista a cui si trova il FRB
    indice=dict_FRBs['name'].index(NOMEFRB)
    #e ottengo le sue coordinate
    ra=dict_FRBs['ra'][indice] 
    dec=dict_FRBs['dec'][indice]
    
    
    print("Clustering sulla posizione di" , dict_FRBs['name'][indice])
    print("RA" , ra)
    print("DEC", dec)
    
    #gtselect sulla posizione del FRB su tutti i dati disponibili e con un raggio di 3°
    ...
    for tempo in lista_deltaT:
        ...
        #salvataggio
        with open('cluster_'+NOMEFRB+'_DT_'+str(tempo)+'.yaml', 'w') as file_yaml:
...
def ricerca_cluster(deltaT,RA,DEC,dati): 
    ...
    #ciclo su tutti gli eventi che verifica la compatibilità spaziale tra essi e la direzione di arrivo del FRB
